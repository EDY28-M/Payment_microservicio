...
public class PaymentsController : ControllerBase
{
    ...
    /// <summary>
    /// Confirma el pago consultando Stripe directamente (fallback cuando no hay webhooks)
    /// </summary>
    [HttpPost("confirm-session/{sessionId}")]
    [Authorize]
    public async Task<ActionResult<PaymentStatusResponse>> ConfirmSessionPayment(string sessionId)
    {
        try
        {
            if (!IsAdmin())
        ...
        {
            _logger.LogError(ex, "Error al confirmar pago de session {SessionId}", sessionId);
            return StatusCode(500, new { mensaje = "Error al confirmar pago" });
        }
    }
