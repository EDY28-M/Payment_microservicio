...
public class PaymentService : IPaymentService
{
    ...
    public async Task<PaymentStatusResponse?> ConfirmSessionPaymentAsync(string sessionId)
    {
        var payment = await _paymentRepository.GetBySessionIdAsync(sessionId);
        if (payment == null)
            return null;

        // Si ya fue procesado, solo retornar estado actual
        if (payment.Status == "Succeeded")
            return MapToResponse(payment);

        // Consultar Stripe directamente para verificar estado real
        try
        {
            var session = await _stripeService.GetSessionAsync(sessionId);
            if (session.PaymentStatus == "paid")
            {
                _logger.LogInformation("[CONFIRM] Stripe confirma pago exitoso para session {SessionId}, procesando...", sessionId);
                await ProcessPaymentCompletedAsync(sessionId);
                
                // Re-leer el pago actualizado
                var updated = await _paymentRepository.GetBySessionIdAsync(sessionId);
                return updated == null ? null : MapToResponse(updated);
            }
            else
            {
                _logger.LogInformation("[CONFIRM] Stripe reporta PaymentStatus={Status} para session {SessionId}", 
                    session.PaymentStatus, sessionId);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CONFIRM] Error al consultar Stripe para session {SessionId}", sessionId);
        }
